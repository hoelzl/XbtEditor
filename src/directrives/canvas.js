...
d2.directive("draw2dCanvas", ["$window", "$parse", "$timeout", function ($window, $parse, $timeout) {
    ...
    return {
        ...
        link: function (scope, element, attrs, controller) {
            ...
            //
            var canvas = new draw2d.Canvas(element.attr("id"),
                scope.editor.canvas.width, scope.editor.canvas.height);
            canvas.setScrollArea("#" + element.attr("id"));
            canvas.onDrop = $.proxy(scope.editor.canvas.onDrop, canvas);

            canvas.installEditPolicy(new draw2d.policy.canvas.SnapToGridEditPolicy(25));
            // TODO: Insertion also needs to respect the grid.
            ...
            // Set the Draw2D router to direct routing
